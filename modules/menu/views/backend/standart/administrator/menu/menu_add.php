...
<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li class=""><a href="<?= site_url('administrator/menu'); ?>">
				<?= cclang('new'); ?>
			</a></li>
		<li class="active">
			<?= cclang('new'); ?>
                      ...
                      'method'  => 'POST'
						...
						<div class="form-group menu-only">
							...
							<div class="col-sm-8">
								...
								<a class="btn btn-default btn-select-icon btn-flat">
									<?= cclang('select_icon') ?>
								</a>

								<select class="chosen  chosen-select-deselect" name="icon_color" id="icon_color"
									tabi-ndex="5" data-placeholder="Select Color">
						...
						<div class="form-group ">
							...
							<div class="col-sm-8">
								<select class="form-control chosen  chosen-select-deselect" name="parent" id="parent"
									tabi-ndex="5" data-placeholder="Select Parent">
									<option value=""></option>
									< <?php foreach (get_menu($this->
										model_menu->get_id_menu_type_by_flag($this->uri->segment(4))) as $row): ?>
										<option value="<?= $row->id; ?>">
											<?= ucwords($row->label); ?>
										</option>
						...
						<div class="form-group ">
							<label for="label" class="col-sm-2 control-label">
								<?= cclang('label') ?> <i class="required">*</i>
							</label>
							<div class="col-sm-8">
						...
						<div class="form-group ">
							<label for="link" class="col-sm-2 control-label">
								<?= cclang('link') ?> <i class="required">*</i>
							</label>
							<div class="col-sm-8">
								<input type="text" class="form-control" name="link" id="link" placeholder="Link"
									value="<?= set_value('link'); ?>">
								<small class="info help-block">The link of menu <i>Example : administrator/blog</i>.
						...
						<div class="form-group ">
							...
							<div class="col-sm-8">
								<label class="col-md-2 padding-left-0">
									<input type="radio" name="type" class="flat-green menu_type" value="menu" checked>
									<?= cclang('menu') ?>
								</label>
								<label>
									<input type="radio" name="type" class="flat-green menu_type" value="label">
									<?= cclang('label') ?>
								</label>
								<small class="info help-block">
									Type Of Menu.
						...
						<div class="form-group ">
							<label for="content" class="col-sm-2 control-label">
								<?= cclang('group_privilage') ?>
							</label>

							<div class="col-sm-8">
								<select class="form-control chosen chosen-select" name="group[]" id="group"
									tabi-ndex="5" data-placeholder="Select Groups" multiple="">
									<option value=""></option>
							<?php
								$group_id = $this->session->userdata('group_id');

								if ($group_id == '1') {
									$data = db_get_all_data('aauth_groups');
								}else{
									$data = db_get_all_data('aauth_groups', 'id NOT IN (1, 2, 3, 4)');
								}
						...
						<div class="row-fluid col-md-7">
							<button class="btn btn-flat btn-primary btn_save btn_action" id="btn_save" data-stype='stay'
								title="save (Ctrl+s)"><i class="fa fa-save"></i>
								<?= cclang('save_button'); ?>
							</button>
							<a class="btn btn-flat btn-info btn_save btn_action btn_save_back" id="btn_save"
								data-stype='back' title="<?= cclang('save_and_go_the_list_button'); ?> (Ctrl+d)"><i
							...
							<a class="btn btn-flat btn-default btn_action" id="btn_cancel"
								title="<?= cclang('cancel_button'); ?> (Ctrl+x)"><i class="fa fa-undo"></i>
								<?= cclang('cancel_button'); ?>
							</a>
							<span class="loading loading-hide"><img
									src="<?= BASE_ASSET; ?>/img/loading-spin-primary.svg"> <i>
