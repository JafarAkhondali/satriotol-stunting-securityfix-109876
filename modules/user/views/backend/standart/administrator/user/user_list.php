				...
				<div class="box-body ">
					...
					<div class="box box-widget widget-user-2">
						...
						<div class="widget-user-header ">
							...
							<?php is_allowed('user_add', function(){?>
								<a class="btn btn-flat btn-success btn_add_new" id="btn_add_new" title="<?= cclang('add_new_button', cclang('user')); ?> (Ctrl+a)" href="<?= site_url('administrator/user/add'); ?>">
									<i class="fa fa-plus-square-o"></i> <?= cclang('add_new_button', cclang('user')); ?>
								</a>
							<?php }) ?>
							<?php is_allowed('user_export', function(){?>
								<a class="btn btn-flat btn-success" title="<?= cclang('export', 'Excel '.cclang('user')); ?>" href="<?= site_url('administrator/user/export'); ?>">
									<i class="fa fa-file-excel-o"></i> <?= cclang('export', 'Excel '.cclang('user')); ?>
								</a>
							<?php }) ?>
							<?php is_allowed('user_export', function(){?>
								<a class="btn btn-flat btn-success" title="<?= cclang('export', 'PDF User'); ?>" href="<?= site_url('administrator/user/export_pdf'); ?>">
							...
							<h5 class="widget-user-desc"><?= cclang('list_all', cclang('user')); ?> <i
									class="label bg-yellow"><?= $user_counts; ?> <?= cclang('items'); ?></i></h5>
						</div>
								...
								<table class="table table-bordered table-striped dataTable">
									<thead>
										<tr class="">
											...
											<th data-field="full_name" data-sort="1">Dinas / Instansi</th>
									...
									<tbody id="tbody_user">
										...
										<tr>
											...
											<td><?php echo _ent($user->opd_nama);?></td>
					...
					<div class="row">
						<div class="col-md-8">
							...
							<div class="col-sm-2 padd-left-0 ">
								<button type="button" class="btn btn-flat" name="apply" id="apply" value="Apply"
									title="<?= cclang('apply_bulk_action', 'User'); ?>"><?= cclang('apply_button', 'User'); ?></button>
							</div>
							<div class="col-sm-3 padd-left-0  ">
								<input type="text" class="form-control" name="q" id="filter" placeholder="Filter"
									value="<?= $this->input->get('q'); ?>">
							</div>
							<div class="col-sm-3 padd-left-0 ">
								<select type="text" class="form-control chosen chosen-select" name="f" id="field">
									<option value=""><?= cclang('all'); ?></option>
									<option <?= $this->input->get('f') == 'id' ? 'selected' :''; ?> value="id">ID
									</option>
									<option <?= $this->input->get('f') == 'username' ? 'selected' :''; ?>
										value="username">Username</option>
									<option <?= $this->input->get('f') == 'full_name' ? 'selected' :''; ?>
										value="full_name">Full Name</option>
									<option <?= $this->input->get('f') == 'email' ? 'selected' :''; ?> value="email">
										Email</option>
								</select>
							</div>
							...
							<div class="col-sm-1 padd-left-0 ">
								<a class="btn btn-default btn-flat" name="reset" id="reset" value="Apply"
									href="<?= base_url('administrator/user'); ?>"
