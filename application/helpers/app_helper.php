...
if (!function_exists('report_builder')) {
	...
	{
		return base_url('administrator/report/preview/' . $report_id . '/' . $row_id);
	}
}
...
if (!function_exists('save_notif_data')) {
	function save_notif_data($data = [], $table = '')
	{
		$ci = &get_instance();
		...
		foreach ((array)@$data['to'] as $to) {
			...
			if ($group) {
				...
				$members = $ci->aauth->list_users($group);
			} else {
				$members = [];
			}
		...
		foreach ((array)@$data['to_user'] as $to) {
		...
		if ((array)@$data['to_query']) {

			$result = $ci->db->query($data['to_query'])->result();
		...
		foreach ((array)@$data['to_department'] as $to) {
			if (is_integer($to)) {
				$group = $to;
			} else {
				@$group = $ci->db->get_where('olympic_master_department', ['Department' => $to])->row()->id;
			}

			$members = $ci->db->get_where('aauth_users', ['department_id' => $to])->result();
		...
		if (count($batch)) {
			$ci->db->insert_batch($table, $batch);
		}
	}
}


if (!function_exists('save_reminder')) {
	function save_reminder($data = [])
	{
		save_notif_data($data, 'data_reminder');
	}
}
...
if (!function_exists('send_notification')) {
	...
	{
		save_notif_data($data, 'notification');
	}
}
