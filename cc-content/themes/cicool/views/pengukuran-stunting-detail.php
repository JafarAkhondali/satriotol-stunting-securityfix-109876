...
<main>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="section__title-wrapper-2 text-center mb-60">
						<h3 class="section__title-2"><?= _ent($pustu[0]->pustun_judul);?></h3>
					</div>
				</div>
			</div>
		</div>
	...
	<section class="certificate__area pb-120 pt-30">
		...
		<?php
			if (!empty($pustu[0]->pustun_media) || $pustu[0]->pustun_media != Null) {
		?>
				<div class="row">
					<div class="col-md-12">
						<div class="certificate__content">
							...
							<?php
								$expl_media = explode(',', $pustu[0]->pustun_media);
								for ($i=0; $i < count($expl_media); $i++) {
							?>
									<div class="item">
										<img src="<?= base_url().'uploads/publikasi_stunting/'.$expl_media[$i];?>"/>
									</div>
							<?php
								}
							?>
